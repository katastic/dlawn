...
class attachedTurret : turret
	{
	float attachmentX = 0; // NYI
	float attachmentY = 0; // offset from 0,0 attachment point so we can rotate around it

	this(float _x, float _y, float _attachX, float _attachY, baseObject _myOwner)
		{
		super(_x, _y, _myOwner);
		isDebugging = true;
		attachmentX = _attachX;
		attachmentY = _attachY;
		}
	...
	override bool draw(viewport v)
		...
		float cx = myOwner.pos.x + v.x - v.ox + cos(cangle)*dist; // NOTE. we're currently NOT USING x,y
		float cy = myOwner.pos.y + v.y - v.oy + sin(cangle)*dist; // because vector addition hard. apparently.
...
class turret : ship
	...
	this(baseObject _myOwner) //, gun _myGun) 
		{
		float _x = _myOwner.pos.x; 
		float _y = _myOwner.pos.y; 
		super(_x, _y, 0, 0);
	...
	override bool draw(viewport v)
		{
		al_draw_centered_bitmap(bmp, myOwner.pos.x + pos.x + v.x - v.ox, myOwner.pos.y + pos.y + v.y - v.oy, 0);
		al_draw_center_rotated_bitmap(turretGun_bmp, myOwner.pos.x + pos.x + v.x - v.ox, myOwner.pos.y + pos.y + v.y - v.oy, angle, 0);
		return true;
		}

	void alignTurretandFire()
		{
		pair absoluteP = pair(myOwner.pos.x + pos.x, myOwner.pos.y + pos.y); 
...
//		assert(destinationAngle == wrapRad(destinationAngle));
		angle = wrapRad(angle);
		destinationAngle = wrapRad(destinationAngle);   // this shouldn't be neede if they're wraprad in angleto above.
