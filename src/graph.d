...
class circularBuffer(T, size_t size) {
	...
	*/
	...
	T opApply(scope T delegate(ref T) dg) { //https://dlang.org/spec/statement.html#foreach-statement
...
class intrinsicGraph(T) {
	...
	void onDraw(viewport v) {
		...
		al_draw_text(g.font1, white, x + w - 32, y + h - g.font1.h, 0, format("%s", min / scaling)
				.toStringz);

		al_draw_text(g.font1, white, x + 32, y + h - g.font1.h, 0, format("%s", dataBuffer.data[dataBuffer.index] / scaling)
				.toStringz);
	}
