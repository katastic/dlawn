...
class dialogConsole {
	...
	void actionScrollUp() {
		clampLow(--scrollIndex, 0);
	}

	void actionScrollDown() {
		clampHigh(++scrollIndex, bufferLength);
	}

	void actionScrollPageUp() {
		scrollIndex -= columnsHeight;
		clampLow(scrollIndex, 0);
	} // we can move less than a full page. like 2/3rd.
...
class logger {
	...
	void log(T)(T obj, string str2) {
		if (!obj.isDebugging)
			return; // If the object isn't set to debug, we ignore it. So we can just set debug flag at will to snoop its data.
		if (echoToStandard)
			writeln(str2);
		if (echoToFile)
