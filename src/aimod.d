...
class bug : BaseObject {
	...
	this(pair _pos) {
		//	gun = gunType();		
		super(_pos, new flatWalkerStyle(cast(BaseObject) this));
		ai = new bugAi(this);
		bmp = bh["beetle"];
	}
...
float angleToward(float currentAngle, float destAngle, float angleChange) {
	...
	writefln("ca:%3.2f da:%3.2f change:%3.2f", currentAngle.radToDeg, destAngle.radToDeg, angleChange
			.radToDeg);
	float result = 0;
	...
	} else if (destAngle < currentAngle) // we need to move backwards across zero
	{
		result = destAngle - currentAngle;
	}
...
class bugAi : aiType {
	...
	void triggerAudioCue(pair triggerPos, float volume) {
		...
		float t = 0;
		if (d < 1000)
			t += 1000 - d; // if within circle, ramp danger
		agitation += t / 10;
