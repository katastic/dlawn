...
class atlasHandler2 : atlasHandlerI {
	...
	final void load(string filepath = "./data/atlasManifest.json") {
		...
		if (js.object is null)
			assert(0, "FAILED TO LOAD JSON MANIFEST FILE."); // is this the right test?
	...
	void processManifest() {
		...
		foreach (filename, entries; filesData) {
			...
			foreach (entryNumber, meta; entries) {
				...
				bmps[meta.tilename] = al_create_sub_bitmap(sources[filename], meta.x, meta.y, meta.w, meta
						.h);
				tileInfo t;
...
class bitmapHandler {
	...
	bitmap* opIndex(string name) {
		...
		if (b is null) {
			...
			if (usePrettyConsole)
				con.log(format("Attempting to get a bitmap [%s] that wasn't loaded!", name));
			else
				writefln("Attempting to get a bitmap [%s] that wasn't loaded!", name);
			assert(false);
		}
		return *b;
	}
	...
	void list() /// List all active bitmaps
	{
		...
		foreach (b; data.byPair)
			writeln(b); //key and pair struct
	}
