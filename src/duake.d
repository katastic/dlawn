...
class duakeCommandLayer /// this is the current command string (and list of previous run commands)
{
	...
	void addCommmandToCommandBuffer(string command) {
		...
		if (previousCommands.length > MAX_STRINGS)
			previousCommands = previousCommands[1 .. $ - 1]; // chop off oldest (first) element
	}
	...
	bool runCommandLine() {
		return true;
	} /// whatever is currently typed in the commandline
}
